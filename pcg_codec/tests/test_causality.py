...
def _stream_roundtrip(x: np.ndarray, hop: int, latent_dim: int, num_blocks: int, block_dim: int) -> np.ndarray:
...
def test_causality_no_lookahead() -> None:
    ...
    y1 = _stream_roundtrip(x, hop=hop, latent_dim=latent_dim, num_blocks=num_blocks, block_dim=block_dim)
    ...
    y2 = _stream_roundtrip(x2, hop=hop, latent_dim=latent_dim, num_blocks=num_blocks, block_dim=block_dim)
